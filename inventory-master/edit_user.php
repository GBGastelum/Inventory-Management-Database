...
?>
<?php
  $e_user = find_by_id('users',(int)$_GET['user_id']);
...
//Update User basic info
  if(isset($_POST['update'])) {
    ...
    if(empty($errors)){
             $id = (int)$e_user['user_id'];
           $f_name = remove_junk($db->escape($_POST['first_name']));
		   $l_name = remove_junk($db->escape($_POST['last_name']));
       $username = remove_junk($db->escape($_POST['username']));
          $level = (int)$db->escape($_POST['level']);
       $status   = remove_junk($db->escape($_POST['status']));
            $sql = "UPDATE users SET first_name ='{$f_name}', last_name = '{$l_name}' username ='{$username}' WHERE user_id='{$db->escape($id)}'";
         $result = $db->query($sql);
          if($result && $db->affected_rows() === 1){
            $session->msg('s',"Acount Updated ");
            redirect('edit_user.php?id='.(int)$e_user['user_id'], false);
          } else {
...
if(isset($_POST['update-pass'])) {
  ...
  if(empty($errors)){
     ...
     $h_pass   = sha1($password);
          $sql = "UPDATE users SET password='{$h_pass}' WHERE user_id='{$db->escape($id)}'";
       $result = $db->query($sql);
        if($result && $db->affected_rows() === 1){
     ...
     <div class="panel panel-default">
       <div class="panel-heading">
        <strong>
          ...
          Update <?php echo remove_junk(ucwords($e_user['first_name'])); ?> Account
        </strong>
       </div>
       <div class="panel-body">
          <form method="post" action="edit_user.php?id=<?php echo (int)$e_user['user_id'];?>" class="clearfix">
            <div class="form-group">
                  <label for="name" class="control-label">Name</label>
                  <input type="name" class="form-control" name="name" value="<?php echo remove_junk(ucwords($e_user['first_name'])); ?>">
            </div>
            <div class="form-group">
  ...
  <div class="col-md-6">
    <div class="panel panel-default">
      <div class="panel-heading">
        <strong>
          <span class="glyphicon glyphicon-th"></span>
          Change <?php echo remove_junk(ucwords($e_user['first_name'])); ?> password
        </strong>
      </div>
      <div class="panel-body">
        <form action="edit_user.php?id=<?php echo (int)$e_user['user_id'];?>" method="post" class="clearfix">
          <div class="form-group">
